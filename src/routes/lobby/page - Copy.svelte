<script>
  ...
  async function checkAllPlayersReady() {
    ...
    try {
      ...
      if (roomData?.admin_id === userId && currentRoom?.status === 'waiting') {
        ...
        // Update room status to starting first
  ...
  async function loadGameSession() {
    try {
      ...
      if (session) {
        ...
        // Load current card if revealed
        if (session.current_card_revealed) {
          console.log('🃏 Loading revealed card data...');
          const cardData = await loadCardData(roomData.game_mode, session.current_card);
          if (cardData) {
            currentCard = cardData;
            console.log('📋 Current card loaded:', currentCard);
          }
        } else {
          console.log('🎴 Card not yet revealed');
          currentCard = null;
        }
      } else {
        console.log('⚠️ No active game session found');
        resetGameState();
      }
    } catch (err) {
  ...
  async function updatePlayerStatus() {
    ...
    try {
      ...
      if (gameStarted) {
        console.log('⏩ Game already started, skipping player status update');
        return;
      }
  ...
  async function handleRoomStateChange(payload) {
    try {
      ...
      if (newRoomData.status === 'playing' && newRoomData.game_session_id) {
        console.log('🎮 Game is now in progress, session ID:', newRoomData.game_session_id);
        ...
        if (!gameStarted) {
          ...
          currentGameState.update(state => ({
            ...state,
            isDrawPhase: true,
            deckName: gameModes[newRoomData.game_mode]?.name || '',
          }));
  ...
  async function syncGameState() {
    try {
      ...
      const { data: room } = await supabase
        .from('rooms')
        .select('status, game_mode, game_session_id')
        .eq('id', roomId)
        .single();

      if (room?.status === 'playing' && room?.game_session_id) {
        console.log('🎮 Room is in playing state, loading session...');
        await loadGameSession();
      } else {
        console.log('⏸️ Room not in playing state:', room?.status);
        resetGameState();
      }
    } catch (err) {
  ...
  async function handleGameStart() {
    ...
    try {
      ...
      console.log('🎲 Attempting to create game session with params:', sessionParams);
      ...
      console.log('✅ Game session created successfully:', newSession);
